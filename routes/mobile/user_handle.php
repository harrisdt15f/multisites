...
Route::match(['post', 'options'], 'login', ['as' => 'mobile-api.login', 'uses' => 'MobileAuthController@login']);
//管理总代用户与玩家
Route::group(['prefix' => 'user'], function () {
    ...
    //用户设置资金密码
    Route::match(['post', 'options'], 'set-fund-password', ['as' => $namePrefix . 'set-fund-password', 'uses' => $controller . 'setFundPassword']);
    //用户个人资料列表
    Route::match(['get', 'options'], 'user-specific-infos', ['as' => $namePrefix . 'user-specific-infos', 'uses' => $controller . 'UserSpecificInfos']);
    //用户设置个人资料
    Route::match(['post', 'options'], 'reset-specific-infos', ['as' => $namePrefix . 'reset-specific-infos', 'uses' => $controller . 'resetSpecificInfos']);
});
