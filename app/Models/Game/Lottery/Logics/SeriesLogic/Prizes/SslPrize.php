...
//##########################################################[时时乐系列开始 prize 计算]#########################################
     ...
     * @return float|int
     */
    private function getPrizeSsl($sFunction, $sBetNumber, $sWnNumber, LotterySeriesWay $oSeriesWay)
    {
        switch ($sFunction) {
            ...
            case 'prizeMultiOneEqual'://返回定位胆的中奖注数
                ...
                break;
            default:
                Log::channel('issues')->info('需要添加时时乐系列方法:'.$sFunction.$oSeriesWay->toJson());
                $result = 0;
        }
        return $result;
    }
