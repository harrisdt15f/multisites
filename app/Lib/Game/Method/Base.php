...
abstract class Base
{
    ...
    public function _expand() {
        if(!$this->supportExpand || empty($this->positionsTpl) || empty($this->expands)) return array($this->id=>$this);
    ...
    public function renew($pos) {
        if(!$this->supportExpand || !$pos) return false;
        if(empty($this->positionsTpl) || empty($this->expands)) return false;
