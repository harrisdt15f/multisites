...
class DigitalNumber
{
     ...
     * @return int
     */
    public static function getShape($sNumber): ?int
    {
     ...
     * @return boolean
     */
    public static function checkCode(& $sCode, $iCodeLen, $sShape = -1): bool
    {
        $sPattern = $sShape == 16 ? '/^[0123]{1,4}\,[0123]{1,4}$/' : '/^\d{'.$iCodeLen.'}$/';
        if (!preg_match($sPattern, $sCode)) {
            return false;
        }
        if ($sShape > -1 && !in_array($sShape, [1, 16])) {
     ...
     * @return string
     */
    public static function getCombinNumber($sNumber, $bUnique = false): string
    {
     ...
     * @return int
     */
    public static function getSum($sNumber): int
    {
        $aWei = str_split($sNumber);
     ...
     * @return int
     */
    public static function getSumTail($sNumber): int
    {
     ...
     * @return int
     */
    public static function getSpan($sNumber): int
    {
        $aWei = str_split($sNumber);
